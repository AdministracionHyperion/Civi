...
## Affiliate booking (ops WhatsApp)

Nearest booking search only returns Civi affiliates (`is_partner` + `booking_mode=civi` + `ops_whatsapp`).

```powershell
$env:PYTHONPATH="services/places-service/src;packages/python-common/src"
# Mark one site:
python -m places_service.cli.manage_presence set-partner `
  --site-id <site_id> --ops-whatsapp 57300XXXXXXX --database-url $env:PLACES_DATABASE_URL

# Or seed 2–3 Santander/Manizales candidates for local tests:
python scripts/seed_partner_affiliates.py `
  --database-url $env:PLACES_DATABASE_URL --ops-whatsapp 57300XXXXXXX
```

Internal APIs: `GET /internal/places/{site_id}/ops-contact`, `POST /internal/places/ops-contact/lookup`.
