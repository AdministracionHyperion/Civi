...
def _is_affiliate_bookable(place: Place) -> bool:
    return (
        bool(getattr(place, "is_partner", False))
        and bool(getattr(place, "is_bookable", False))
        and str(getattr(place, "booking_mode", "") or "") == "civi"
    )
...
def _is_searchable(place: Place) -> bool:
    ...
    return status not in EXCLUDED_STATUSES and _is_affiliate_bookable(place)
    ...
    *,
    ...
    if payload.lat is not None and payload.lng is not None:
        ...
        return FindNearestPlaceResponse(
            ...
            match_scope="gps",
            resolved_location={"lat": payload.lat, "lng": payload.lng},
            no_results_reason=None if places else "no_affiliates_within_radius",
            search_radius_km=radius_km,
    ...
    if city:
        ranked_places = sorted(
            candidates,
            key=lambda place: (place.city, place.name),
        )
        ...
        return FindNearestPlaceResponse(
            ...
            no_results_reason=None if places else "no_affiliates_in_municipality",
            search_radius_km=radius_km,
