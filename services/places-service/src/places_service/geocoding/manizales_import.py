...
"""Validated Manizales geocode import — validation + apply core.

The CLI (``places_service.cli.import_manizales_geocodes``) is a thin wrapper around
:func:`run_import`. Everything here is import-safe and unit-testable.
...
Design rules (see PLACES_OPERATIONS_RUNBOOK.md):
  ...
  * Resolution is strict: official CSV ``id`` -> ``places_sites.source_place_id``
    (with an optional ``places_source_records`` fallback). Never fuzzy-match,
    never create new sites.
...
from places_service.domain.models import (
    CONFIRMED_VALIDATION_STATUSES,
    GEOCODE_VALIDATION_STATUSES,
    LOCATION_PRECISIONS,
)
from places_service.geocoding.bounds import MANIZALES_BBOX
...
REQUIRED_COLUMNS = frozenset(
    {"id", "name", "address", "city", "department", "country", "kind", "lat", "lng",
     "validation_status", "confidence", "precision", "provider"}
)
...
def _sanitize_evidence(value: str | None, *, max_len: int = EVIDENCE_MAX_LEN) -> str | None:
    if not value:
        return None
    # Strip control characters and collapse whitespace before truncating.
...
def validate_rows(
...
) -> tuple[list[ManizalesGeocode], list[dict[str, Any]]]:
    """Validate every row. Returns (valid_rows, errors). Never writes."""
    valid: list[ManizalesGeocode] = []
    ...
    for offset, raw in enumerate(raw_rows):
        ...
        if country.casefold() != EXPECTED_COUNTRY:
            row_errors.append(f"country_not_colombia:{country or '<empty>'}")
        if department.casefold() != EXPECTED_DEPARTMENT:
            row_errors.append(f"department_not_caldas:{department or '<empty>'}")
        if city.casefold() != EXPECTED_CITY:
            row_errors.append(f"city_not_manizales:{city or '<empty>'}")
        ...
        if lat is not None and lng is not None and not MANIZALES_BBOX.contains(lat, lng):
            row_errors.append(f"outside_manizales_bbox:{lat},{lng}")
...
class Classification:
    to_write: list[ManizalesGeocode] = field(default_factory=list)
...
def _same_geocode(site: dict[str, Any], row: ManizalesGeocode) -> bool:
    return (
    ...
    *,
    ...
    with repo.engine.connect() as conn:
        for row in rows:
            ...
            if _same_geocode(dict(site), row):
                result.unchanged += 1
                continue
            ...
            if site.get("geocode_status") == "manual" and has_coords and not force_manual:
                ...
                result.blocked.append(
                    {"row": row.row_number, "id": row.source_place_id, "site_id": site_id,
...
def _write_rows(conn, rows: list[ManizalesGeocode], *, now: str) -> dict[str, int]:
        ...
        conn.execute(
            places_geocode_attempts.insert().values(
                ...
                response_payload=json.dumps(
                    {
                        "source": "manizales_validated_geocode",
...
def _base_report(*, mode: str, input_path: Path, valid: list[ManizalesGeocode], raw_count: int) -> dict[str, Any]:
    return {
        "mode": mode,
...
def run_import(
    *,
    ...
    """Run the Manizales import. Returns (exit_code, report). Writes report if path given."""
    now = now or datetime.now(timezone.utc).isoformat()
    try:
        raw_rows = read_csv_rows(input_path)
    except OSError as exc:
        report = {"mode": "apply" if apply else "dry_run", "error": f"infra: {exc}",
                  "atomic_aborted": True}
    ...
    mode = "apply" if apply else "dry_run"
    report = _base_report(mode=mode, input_path=input_path, valid=valid, raw_count=len(raw_rows))

    # Roll up validation errors for the report.
    ...
    report["outside_bbox"] = sum(
        1 for e in errors if any(str(r).startswith("outside_manizales_bbox") for r in e["reasons"])
    )
    ...
    # CSV-only path (dry-run without a database): validation already passed.
    if not database_url:
        report["inserted"] = len(valid)  # would-be inserts, DB not consulted
        _maybe_write_report(report_path, report)
    ...
    # APPLY path — atomic. Any blocking aborts before writing.
    if has_blocking:
        report["atomic_aborted"] = True
        _maybe_write_report(report_path, report)
        return EXIT_VALIDATION, report
    ...
    try:
        with repo.engine.begin() as conn:
            write_counts = _write_rows(conn, classification.to_write, now=now)
    except Exception as exc:  # noqa: BLE001
        report["atomic_aborted"] = True
        report["error"] = f"infra: {exc}"
        _maybe_write_report(report_path, report)
